    ...
    try {
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: questionExtractionResponseSchema,
          // 문항이 많은 문제지는 출력이 길어져 기본 토큰 한도에서 JSON이 잘릴 수 있어 최대치로 늘림.
          maxOutputTokens: 65536,
          // 기본 thinking 모드는 실제 문제지 PDF에서 6분 가까이 걸려 Vercel 함수 시간 제한을 초과한다.
          // 추출 작업은 깊은 추론보다 옮겨 적기에 가까우므로 thinking 예산을 제한해 응답 시간을 줄인다.
      ...
      try {
        ...
        throw new GradingError("Gemini 응답이 중간에 잘렸거나 JSON 형식이 아닙니다.");
      }
    } catch (err) {
      lastError = err;
...
export async function extractQuestionsFromPdf(pdfBuffer: Buffer): Promise<QuestionExtractionItem[]> {
  const parts: Array<{ text: string } | { inlineData: { data: string; mimeType: string } }> = [
    { inlineData: { data: pdfBuffer.toString("base64"), mimeType: "application/pdf" } },
    { text: INSTRUCTIONS },
  ];

  // callGemini가 던지는 에러(응답 잘림, 레이트리밋 소진 등)도 스키마 검증 실패와 동일하게
