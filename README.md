...
1. https://aistudio.google.com/apikey 에서 API 키를 만듭니다.
2. **실제 여러 명이 쓸 거라면 결제(billing)를 연결**하는 것을 권장합니다. 무료 등급은 분당/일일 호출
   한도가 낮아서, 여러 선생님이 동시에 PDF를 채점하면 금방 막힙니다. Gemini flash 모델은 단가가 낮아
   소규모 학교면 비용이 크지 않습니다.

## 4단계. Vercel에 배포

1. https://vercel.com 에 GitHub로 로그인 → **Add New → Project** → 1단계에서 만든 저장소 선택.
2. **Environment Variables**에 아래를 입력합니다 (값은 Supabase Project Settings → API, Gemini는 3단계에서):
